...
public class UserServiceTest {
    ...
    void registerUser_success() throws DataAccessException {
        ...
        assertNotNull(result.user().username());
        assertNotNull(result.user().password());
        assertNotNull(result.user().email());
        assertEquals("newuser", result.user().username());
        assertEquals("password123", result.user().password());
        assertEquals("email@example.com", result.user().email());
    }
